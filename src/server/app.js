...
class App {
  ...
  signUp() {
    this.app.post('/sign-up', (req, res) => {
      ...
      users.push(req.body);
      fs.writeFileSync('data/users.json', JSON.stringify(users)); //методы модуля fs позволяют писать данные в файл. таким образом они попадают из файла users.json к нам в переменные
